...
case $(echo ${_command}) in
  ...
  start)
    if [[ ${dry_run} -eq 1 ]]; then
      ...
      [[ ${py_read} -eq 1 ]] && write_magenta "Dry-Run> nohup python3 ${BOK_GALIL_BIN}/${_prc2} >> ${BOK_GALIL_LOG}/${_prc2}.log 2>&1 &"
      ...
      if [[ ${py_read} -eq 1 ]]; then
        write_green "Executing> nohup python3 ${BOK_GALIL_BIN}/${_prc2} >> ${BOK_GALIL_LOG}/${_prc2}.log 2>&1 &"
        [[ -z ${_pid2} ]] && (nohup python3 ${BOK_GALIL_BIN}/${_prc2} >> ${BOK_GALIL_LOG}/${_prc2}.log 2>&1 &) || write_error "${_nam2}" "ALREADY RUNNING"
        write_green "Executing> sleep 1"
        sleep 1
      fi
      ...
      if [[ ${py_read} -eq 1 ]]; then
        write_green "Executing> nohup python3 ${BOK_GALIL_BIN}/${_prc5} >> ${BOK_GALIL_LOG}/${_prc5}.log 2>&1 &"
        [[ -z ${_pid5} ]] && (nohup python3 ${BOK_GALIL_BIN}/${_prc5} >> ${BOK_GALIL_LOG}/${_prc5}.log 2>&1 &) || write_error "${_nam5}" "ALREADY RUNNING"
        write_green "Executing> sleep 1"
        sleep 1
      fi
