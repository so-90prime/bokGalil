...
def read_mmap_to_dict(_m: mmap.mmap = None, _verbose: bool = False) -> dict:
    ...
    try:
        ...
        _tcp_d['lv_fnum'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        _tcp_d['lv_fnum_in'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        ...
        _tcp_d['lv_gfiltreq'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        _tcp_d['lv_gfiltsp'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        _tcp_d['lv_gfsp'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        _tcp_d['lv_gifltn'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        ...
        _tcp_d['lv_snum'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
        _tcp_d['lv_snum_in'] = float(f"{struct.unpack('f', _m.read(4))[0]}")
