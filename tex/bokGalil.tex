...
\noindent \emph{Document revision date 2025/11/11. Last author: David Sand (dsand@arizona.edu).} \\
\end{abstract}
...
\noindent {\bf Prelude: } A couple of basics that may seem obvious, but we put here for completeness:

\begin{enumerate}

\item Before you sit down in the control room, {verify that the manual power switch on the archon controller is in the `on' position.}

\item Start up two Firefox windows on bart (the world icon in the bottom tool bar).

\item In one tab, go to Outlet Control: \sfmagenta{10.30.3.30/index.htm}.  Only the following objects should be in the `ON' state: a) Galil Firewall, b) Galil, c) Camera Shutter, d) New 90Prime CCD, e) Guider Raspberry Pi, f) Guider Camera.



\end{enumerate}

\noindent To start the system, first start a terminal window.  This is the black box with the `\$' sign in the bottom control panel.   When that window pops up, type the following if operating on \sfmagenta{bonsai (10.30.1.7)}\footnote{If running from
...
\emteal{source \hspace{1mm} etc/bokGalil.sh \hspace{1mm} \$(pwd) \hspace{1mm} gui}
%\footnote{
...
to appear---indicating that startup is complete---is a \emph{ds9} window.  In two separate Firefox windows (so you can see them side by side) type in the following URLs, which should look like those shown in Figure~\ref{bokgui}: \\

\ttblue{http://10.30.1.7:5905}  [this is meant for astronomers], and

\ttblue{http://10.30.1.7:5905/indi} [this is meant for Mtn Ops and engineers] \\
...
\noindent To stop the system, press the \emteal{stop} button in the first GUI that you opened up. The system status can be interrogated at any time by pressing 
...
The day crew should also start AzCam by double clicking on the 90Prime 'star' icon on the Azcam computer.  Sometimes AzCam takes a while to get started, but once loaded there are six observing modes.  The day crew should select \underline{option 6: New 90Prime (archon)}.  So that the camera is ready for the observers, please also 1) hit the \underline{Reset button} so that the temperature properly displays; 2) make sure that the Filename is changed so data goes to `/nfs/data/primefocus/20250918' where the last directory corresponds to the current date, {\it yyyymmdd}; 3) please set the Sequence number to 1.
...
\noindent where \emph{yyyymmdd} is the observation date associated with the data directory shown in section~\ref{datadirectory} on page~\pageref{datadirectory}. You will then have `ccdlist', `imexam' \emph{etc} available within the iraf window.

\subsection{Filter File(s)}
\label{filterfiles}

\noindent The file containing all of the 90Prime filters is in a different spot for the new 90Prime; they are now on bonsai/bonzai.  The list(s) of available filters are stored in separate files in \$BOK\_GALIL\_DOCS:
...
\noindent For the latter utility, there is a JSON-like syntax required by the code. As an example we can fix the OBSERVER keyword and add a new OBSCOMM keyword in the test.fits file like so:
...
\noindent The OBSCOMM keyword is particularly useful since the bok\_observer\_log.py utility will pick it up and, if it's not 
