...
\begin{abstract}
This document describes the new software and hardware systems to suppport the upgraded 90Prime imager at 
...
\noindent To start the system, execute the following if operating on \sfmagenta{bonsai (10.30.1.7)}\footnote{The commands are 
exactly the same from \sfmagenta{banzai (10.30.1.8)} but with the different IP address!}: \\

\emteal{ssh -XY primefocus@10.30.1.7}

\emteal{cd /home/primefocus/bokGalil}

\emteal{source /home/primefocus/bokGalil/etc/bokGalil.sh \$(pwd) gui} \\

\noindent Allow the interface shown in Figure~\ref{boktcl} to appear and press the \emteal{start} button. Then choose an 
appropriate web interface as shown in Figure~\ref{bokgui}: \\

\emteal{http://10.30.1.7:5905} for astronomers, or

\emteal{http://10.30.1.7:5905/indi} for engineers. \\
...
the \emteal{status} button. Figure~\ref{bokgui} shows the system completely stopped on the left-hand side (all red) and
...
accessed via the engineering interface (http://10.30.1.7:5905/indi, and see Figure~\ref{engineer}):

\begin{description}
 \item[{\sc populate}] This command, once executed, allows the day crew to install various filters using the side button on the dewar.
...
\begin{description}
\item[dataserver] https://github.com/so-mops/dataserver.git \\
\item[bok-90prime-gui] https://github.com/so-90prime/bok-90prime-gui.git \\
\item[pyINDI] git+https://github.com/MMTObservatory/pyINDI.git \\
\end{description}
...
\begin{description}
 \item[INDI / pyINDI] is the infrastructure software. The \emph{indiserver} can be installed following the intructions in
       Appendix~\ref{AppendixA}. The pyINDI software from the MMT is installed direct from a GitHub repository.
